...
def auto_scale_block(module, module_kwargs, w_bit, q_config, input_feat):
    ...
    elif module.__class__.__name__ == "DiTBlock":
        # Wan2.1 video DiT block (DVD). Only mathematically-exact fusions:
        #
        # cross-attn q <- norm3 (affine LayerNorm). Note: only cross_attn.q
        # consumes norm3(x) -- CrossAttention.forward computes
        # q = norm_q(self.q(x)) from the block's own residual stream, but
        # k = norm_k(self.k(ctx)) and v = self.v(ctx) consume the external
        # `context` argument, not norm3's output. Fusing a scale into
        # cross_attn.k/v here would be unsound (nothing in this block
        # compensates the extra factor on their weights), so this group is
        # norm3 -> cross_attn.q alone.
        scales_list.append(
            _auto_get_scale(
                prev_op=module.norm3,
                layers=[module.cross_attn.q],
                inp=input_feat["cross_attn.q"],
            )
        )
        # attn out <- v (attention output is per-channel-linear in v: the
        # softmax attention weights don't depend on v, so scaling each
        # output channel of v by s scales that channel of the attention
        # output by exactly s).
        scales_list.append(
            _auto_get_scale(
                prev_op=module.self_attn.v,
                layers=[module.self_attn.o],
                inp=input_feat["self_attn.o"],
            )
        )
        scales_list.append(
            _auto_get_scale(
                prev_op=module.cross_attn.v,
                layers=[module.cross_attn.o],
                inp=input_feat["cross_attn.o"],
            )
        )
        # ffn down-proj <- GELU (apply_scale wraps the GELU in a
        # ScaledActivation that divides its output by the scale at runtime)
        scales_list.append(
            _auto_get_scale(
                prev_op=module.ffn[1],
                layers=[module.ffn[2]],
                inp=input_feat["ffn.2"],
            )
        )
